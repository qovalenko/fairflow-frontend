...
import NotificationToaster from '@/components/template/NotificationToaster'
import ProductionLaunchBanner from '@/components/template/ProductionLaunchBanner'
import InviteAcceptanceBanner from '@/components/template/InviteAcceptanceBanner'
...
const PostLoginLayout = ({ layoutType, children }: PostLoginLayoutProps) => {
    ...
    return (
        ...
        >
            <AppLayout>
                {/* Прод-launch баннер «идёт подготовка к запуску» (self-gated) — самый верх. */}
                <ProductionLaunchBanner />
                <InviteAcceptanceBanner />
                {children}
            </AppLayout>
            {/* SCR-NOTIFY-TOAST — глобальный overlay нового уведомления (self-gated) */}
            <NotificationToaster />
        </Suspense>
    )
}
