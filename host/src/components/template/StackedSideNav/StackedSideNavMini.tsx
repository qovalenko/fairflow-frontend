...
const StackedSideNavMini = (props: StackedSideNavMiniProps) => {
    ...
    const handleMenuItemSelect = ({
        ...
        onChange({ key, title, menu, translateKey })
    ...
    const handleExpand = () => {
        const navToOpen =
            includedRouteTree?.key &&
            includedRouteTree.subMenu?.length
                ? includedRouteTree
                : navigationTree.find((n) => n.subMenu?.length)
        if (navToOpen) {
            handleMenuItemSelect({
                key: navToOpen.key,
                title: navToOpen.title,
                menu: navToOpen.subMenu,
                translateKey: navToOpen.translateKey,
            })
        }
    }

    const isCollapsed = isEmpty(selectedMenu)

    return (
        <div className={classNames('flex flex-col h-full', className)} style={style} {...rest}>
            <Link
            ...
            <ScrollBar autoHide direction={direction} className="flex-1 min-h-0">
                <Menu
                    className="px-4 pb-4"
            ...
            {isCollapsed && (
                <button
                    type="button"
                    className="close-button mx-auto my-3 flex"
                    aria-label="Открыть меню"
                    onClick={handleExpand}
                >
                    {direction === DIR_LTR && <PiCaretRightDuotone />}
                    {direction === DIR_RTL && <PiCaretLeftDuotone />}
                </button>
            )}
        </div>
    )
