...
const BasePicker = (props: BasePickerProps) => {
    const {
        ...
        inputLabel,
        inputPrefix,
        inputSuffix = <PiCalendarDuotone className="text-lg" />,
        name,
        onDropdownOpen,
        onDropdownClose,
        onBlur,
    ...
    const calendarIcon = inputSuffix ?? (
        <PiCalendarDuotone className="w-4 h-4 text-gray-500 dark:text-gray-400 shrink-0" />
    )
    const clearButtonSlot =
        clearable && inputLabel
            ? clearButton ? (
    ...
    const prefixIconSlot = inputPrefix ?? calendarIcon
    const suffixIconSlot = clearButtonSlot
