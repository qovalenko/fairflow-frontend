...
export interface PaginationProps extends CommonProps {
    ...
    pageSize?: number
    total?: number
    /** Prefix for e2e qa-ids, e.g. `deals.list.pagination` → `.next`, `.page`. */
    qaPrefix?: string
    /** Префикс data-qa-id для стрелок «назад/вперёд» (T-028), например `companies.list.pagination`. */
    qaScope?: string
    nextQa?: QaAttributes
}
    ...
    return (
        <div className={paginationClass}>
            ...
            <Prev
                ...
                qaId={qaScope ? `${qaScope}.prev` : undefined}
                onPrev={onPrev}
            />
            <Pager
            ...
            <Next
                ...
                pageCount={getInternalPageCount as number}
                pagerClass={pagerClass}
                qaId={qaScope ? `${qaScope}.next` : undefined}
                onNext={onNext}
                qaPrefix={qaPrefix}
                {...nextQa}
            />
